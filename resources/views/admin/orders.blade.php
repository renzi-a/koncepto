    ...
    <div class="container mx-auto px-4 py-6 space-y-6">
                ...
                <li>
                    <a href="?tab=all"
                       class="{{ $tab == 'all' 
                                ...
                                : 'text-gray-600 hover:text-green-600' }} font-semibold pb-2 flex items-center space-x-1">
                    ...
                    <button
                        class="px-4 py-1.5 rounded text-sm font-semibold
                            {{ request('status') == $status 
                                || (request('status') == null && $status == 'All') 
        ...
        <div class="space-y-4">
            @forelse ($orders as $order)
                <div class="{{ $order->is_custom 
                        ? 'bg-yellow-50 border-l-4 border-yellow-400' 
                        : 'bg-blue-50 border-l-4 border-blue-400' }} rounded-xl shadow p-4">
                    ...
                    <div class="flex justify-between items-stretch mb-2">
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">
                                <!-- START: Updated logic to show progress bar only for custom orders in 'gathering' status -->
                                @if($order->is_custom && strtolower($order->status) === 'gathering')
                                    @php
                                    ...
                                    <span class="block text-sm font-medium text-gray-700 mb-1">
                                        Gathered: {{ $order->gathered_items_count }} out of {{ $order->items_count }}
                                    </span>
                                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                                        <div class="h-2.5 rounded-full transition-all duration-500 {{ $progressColor }}" style="width: {{ $progress }}%"></div>
                                    </div>
                                @else
                        ...
                        <div class="flex flex-col justify-center items-end">
                            <div class="flex flex-col items-end space-y-2">
                                <div class="flex space-x-2">
                                    <span class="inline-block {{ $order->is_custom 
                                ...
                                @if ($order->is_custom)
                                    @if (strtolower($order->status) === 'to be quoted')
                                        <a href="{{ route('admin.custom-orders.quotation', $order->id) }}"
                                           class="text-xs inline-block px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition">
                                            Start Quotation
                                        </a>
                                    @elseif (strtolower($order->status) === 'approved')
                                        <p class="text-xs mt-2 text-green-600 italic">
                                            Approved. Awaiting gathering.
                                        </p>
                                    @endif
                                @endif
                            </div>
                    ...
                    <div class="flex justify-between items-center mt-4 text-sm">
                        <a href="{{ 
                            ...
                            {{ strtolower($order->status) === 'approved' && $order->is_custom 
                                ? 'View Approved Order' 
                                : (strtolower($order->status) === 'processing' && !$order->is_custom ? 'Start Gathering' : 'View Details') }}
                        </a>
