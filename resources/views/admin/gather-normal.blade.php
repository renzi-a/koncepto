<x-layout>
    <div class="container mx-auto px-6 py-8 space-y-6"
        ...
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-bold text-gray-800">Gathering Items for Order #{{ $order->id }}</h1>
            <a href="{{ route('admin.orders') }}" class="text-sm text-blue-600 hover:underline">
                ← Back to Orders
            </a>
        </div>

        <p class="text-sm text-gray-700 mb-4">
            Gathered: <strong class="text-green-600" x-text="gatheredItems"></strong> of
        ...
        <div class="bg-white p-6 rounded-xl shadow">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-gray-600">
                            <strong>School:</strong> {{ $order->user->school->school_name ?? 'N/A' }}
                        </p>
                        <p class="text-sm text-gray-600">
                            <strong>School Admin:</strong> {{ $order->user->first_name ?? '-' }} {{ $order->user->last_name ?? '' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="overflow-x-auto mt-4">
                <table class="min-w-full table-auto border text-sm text-gray-700 whitespace-nowrap">
                            ...
                            <tr class="border-t transition-colors"
                                ...
                                <td class="px-4 py-2 text-center">
                                    <input type="checkbox"
                                        x-model="isGathered"
                                        @change="
                                            // The key fix: This is how we update the parent scope's variable
                                            if (isGathered) {
                                                $root.gatheredItems++;
                                            } else {
                                                $root.gatheredItems--;
                                            ...
                                            fetch('{{ route('admin.orders.toggle-gathered', $item->id) }}', {
                                                method: 'POST',
                                                headers: {
            ...
            <form action="{{ route('admin.orders.gather.store', $order->id) }}" method="POST">
                @csrf
                ...
                <template x-if="showModal">
                    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                        <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm">
                            ...
                            <p class="text-gray-600 mb-6">Are you sure all items have been gathered?</p>
                            <div class="flex justify-end gap-4">
                                <button @click="showModal = false" type="button" class="text-gray-600 hover:text-gray-800">
                                    Cancel
                                </button>
