        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="w-full lg:w-1/2 flex flex-col">
                <div class="space-y-3">
                    ...
                    <p class="text-gray-700 leading-relaxed text-base pt-2">{{ $product->description }}</p>
                    ...
                    <div class="pt-4 space-y-2">
                        <p class="text-gray-700 flex items-center">
                            ...
                            Category: <span class="font-semibold text-gray-900 ml-1">{{ $product->category->categoryName ?? 'N/A' }}</span>
                        </p>
                        <p class="text-gray-700 flex items-center">
                            <svg class="w-4 h-4 mr-2 text-gray-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M5.5 3A2.5 2.5 0 003 5.5v2.293l3.546 3.547A1 1 0 007 11.414V15a1 1 0 001 1h4a1 1 0 001-1v-3.586a1 1 0 00-.293-.707L12 7.793V5.5A2.5 2.5 0 009.5 3h-4zM2 15h16v1a1 1 0 01-1 1H3a1 1 0 01-1-1v-1z" clip-rule="evenodd"></path>
                            </svg>
                            Stocks Available: <span class="font-semibold text-gray-900 ml-1">{{ $product->quantity }} {{ $product->unit }}</span>
                        </p>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="flex items-center gap-2">
                            <button type="button" onclick="decreaseQty()"
                                class="w-8 h-8 flex items-center justify-center border border-gray-300 rounded-md text-lg text-gray-700 hover:bg-gray-100 transition duration-150">−</button>
                            <input id="qtyInput" name="quantity" type="number" value="1" min="1" max="{{ $product->quantity }}"
                                   data-price="{{ $product->price }}" data-max="{{ $product->quantity }}"
        ...
        <div class="max-w-6xl mx-auto mt-12 p-6 bg-white rounded-lg shadow-md">
            ...
            <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                @foreach ($similarProducts as $item)
                    <a href="{{ route('view_product', $item->id) }}" class="group relative block bg-white border border-gray-100 p-2 rounded-md shadow-sm hover:shadow-md transition-transform duration-200 ease-in-out transform hover:scale-105">
    ...
    @if($relatedProducts->isNotEmpty())
        <div class="max-w-6xl mx-auto mt-12 p-6 bg-white rounded-lg shadow-md">
            ...
            <div x-data="{
                ...
                currentOffset: 0,
                itemsPerPage: 1,
                get maxOffset() { return Math.max(0, this.products.length - this.itemsPerPage); },
                next() { this.currentOffset = Math.min(this.currentOffset + 1, this.maxOffset); },
                prev() { this.currentOffset = Math.max(this.currentOffset - 1, 0); },
                updateItemsPerPage() {
                    if (window.innerWidth >= 1280) this.itemsPerPage = 4;
                    else if (window.innerWidth >= 1024) this.itemsPerPage = 3;
                    else if (window.innerWidth >= 768) this.itemsPerPage = 2;
                    else this.itemsPerPage = 1;
                    if (this.currentOffset > this.maxOffset) this.currentOffset = this.maxOffset;
                },
                init() { this.updateItemsPerPage(); window.addEventListener('resize', () => this.updateItemsPerPage()); }
            }" x-init="init()" class="relative overflow-visible">

                <div class="relative overflow-hidden">
                    <div class="flex transition-transform duration-300 ease-out"
                         :style="`transform: translateX(-${(currentOffset * 100) / itemsPerPage}%)`">
                        <template x-for="item in products" :key="item.id">
                            <a :href="'{{ url('user/view_product') }}/' + item.id"
                               :class="{
                                    'w-full': itemsPerPage === 1,
                                    'sm:w-1/2': itemsPerPage === 2,
                                    'md:w-1/2': itemsPerPage === 2,
                                    'lg:w-1/3': itemsPerPage === 3,
                                    'xl:w-1/4': itemsPerPage === 4,
                                    'flex-shrink-0': true,
                                    'px-2': true
                               }"
...
<div x-data="{ show: false, message: 'Added to cart!' }"
     x-init="window.addEventListener('show-cart-toast', (event) => { show = true; message = event.detail.message || 'Added to cart!'; setTimeout(() => show = false, 2500); })"
     x-show="show"
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    qtyInput.addEventListener('input', () => {
        ...
        else if (value > maxQuantity) qtyInput.value = maxQuantity;
        formQtyInput.value = qtyInput.value;
    });
});
...
function buyNow(productId) {
    ...
    form.innerHTML = `<input type="hidden" name="product_id" value="${productId}"><input type="hidden" name="quantity" value="${qty}">`;
